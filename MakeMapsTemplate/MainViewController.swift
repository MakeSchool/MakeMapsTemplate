...
class MainViewController: UIViewController, MKMapViewDelegate,CLLocationManagerDelegate {
    ...
    private func updatePingAndMap(){
        if passLat != nil && passLong != nil {
            ...
            if  custom  != true{
                print("Indicates not custom", appendNewline: false)
                longitude =  passLong
                latitude =   passLat
                
            } else {
                print("Indicates  custom", appendNewline: false)
                longitude =  long
                latitude =   lat
            }
    ...
    func locationManager(manager: CLLocationManager, didUpdateLocations locations: [AnyObject]) {
        CLGeocoder().reverseGeocodeLocation(manager.location!) {(placemarks, error)-> Void in
            ...
            if let errors = error {
                print("Reverse geocoder failed with error %@", errors.localizedDescription)
                return
            }
